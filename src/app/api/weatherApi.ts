...
export async function getWeatherByCity(lat: number, lon: number): Promise<WeatherData> {
...
return {
  ...
  periods: {
    dawn: {
      ...
      icon: dawn?.condition.icon ? `https:${dawn.condition.icon}` : null,
    },
    morning: {
      time: '09:00',
      temp: morning?.temp_c ?? null,
      icon: morning?.condition.icon ? `https:${morning.condition.icon}` : null,
    },
    afternoon: {
      time: '15:00',
      temp: afternoon?.temp_c ?? null,
      icon: afternoon?.condition.icon ? `https:${afternoon.condition.icon}` : null,
    },
    night: {
      time: '21:00',
      temp: night?.temp_c ?? null,
      icon: night?.condition.icon ? `https:${night.condition.icon}` : null,
    },
  },
};
}
...
export const fetchWeatherData = async (city: City): Promise<WeatherData> => {
  console.log('fetchWeatherData received city:', city);
